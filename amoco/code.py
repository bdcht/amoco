...
class block(acode):
    ...
    def cut(self,address):
        ...
        try:
            ...
            logger.warning(u"invalid attempt to cut block @%s at %s"%(self.address,address))
            return 0
        else:
    ...
    def __repr__(self):
        ...
        nbi = len(self.instr)
        return u'<{} object ({}-{}) with {} instructions>'.format(self.__class__.__name__,
                                                                  sta,sto,
                                                                  nbi)
    def raw(self):
...
class func(acode):
    ...
    def blocks(self):
        ...
        """
        return sorted(filter(lambda x: x._is_block,
                      [n.data for n in self.cfg.sV]),
    ...
    def __str__(self):
        return u"%s{%d}"%(self.address,len(self.blocks))
