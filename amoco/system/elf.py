    ...
    def __init__(self,f):
        ...
        if self.Ehdr.e_phoff:
            ...
            for pht in range(n):
                ...
                offset += l
                if P.p_type == PT_LOAD:
                    if not self.basemap: self.basemap = P.p_vaddr
                    self.Phdr.append(P)
...
class Shdr(StructFormatter):
    def __init__(self,data=None,offset=0,order=None,x64=False):
        if order:
            for f in self.fields: f.order = order
        if x64:
...
class Phdr(StructFormatter):
    def __init__(self,data=None,offset=0,order=None,x64=False):
        ...
        if x64:
            ...
            for f in self.fields[2:]: f.typename = 'Q'
