...
class Elf32_Ehdr(Elfcore):
    ...
    def __str__(self):
        c = ["%s%-12s:%s"%(self.pfx,k,str(getattr(self,k))) for k in self.keys]
        c[3] = "%s%-12s:%s"%(self.pfx,'e_entry',hex(self.e_entry))
        return "%s\n"%self.e_ident + '\n'.join(c)
...
SHT_legal=[eval(v) for v in filter( (lambda s: s.startswith('SHT_')), dir())]
...
PT_legal=[eval(v) for v in filter( (lambda s: s.startswith('PT_')), dir())]
...
DT_legal=[eval(v) for v in filter( (lambda s: s.startswith('DT_')), dir())]
        ...
        if self.Ehdr.e_phoff:
            ...
            for pht in range(n):
                ...
                if self.Phdr[-1].p_type == PT_LOAD:
                    ...
                    self.dynamic = True
                elif not self.Phdr[-1].p_type in PT_legal:
    ...
    def __symbols(self,type):
        ...
        if self.readsection('.strtab'):
            for sym in self.symtab:
                if sym.ELF32_ST_TYPE()==type and sym.st_value:
