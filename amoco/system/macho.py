...
class MachO(BinFormat):
    ...
    def __read_bind_opcodes(self,raw,default_type=0):
        ...
        while cur<len(raw):
            ...
            if   op == BIND_OPCODE_DONE:
                ...
                if nulchar>cur: r.symbol = raw[cur:nulchar]
                cur = nulchar+1
    ...
    def __read_dysymtab(self,cmd):
        ...
        t.toc = self.__read_table(cmd.tocoff,
                                struct_dylib_table_of_contents,
                                cmd.ntoc)
        # read module table:
        if self.header.magic==MH_MAGIC_64:
        ...
        t.modtab = self.__read_table(cmd.modtaboff,
                                   elt,
                                   cmd.nmodtab)
        t.extrefsym = self.__read_table(cmd.extrefsymoff,
                                   struct_dylib_reference,
                                   cmd.nextrefsyms)
        t.indirectsym = self.__read_table(cmd.indirectsymoff,
                                        struct_indirect_entry,
                                        cmd.nindirectsyms)
        t.extrel = self.__read_table(cmd.extreloff,
                                   struct_relocation_info,
                                   cmd.nextrel)
        t.locrel = self.__read_table(cmd.locreloff,
                                   struct_relocation_info,
                                   cmd.nlocrel)
        return t
...
class struct_nlist(MachoFormatter):
    ...
    def ntype_fmt(self,k,x,cls=None):
        if x & N_STAB:
            ...
            l = []
            if   x&N_TYPE == 0x0: l.append('N_UNDF')
            elif x&N_TYPE == 0x2: l.append('N_ABS')
            elif x&N_TYPE == 0xe: l.append('N_SECT')
            elif x&N_TYPE == 0xc: l.append('N_PBUD')
            elif x&N_TYPE == 0xa: l.append('N_INDR')
            if x & N_EXT: l.append('N_PEXT')
            s = '+'.join(l)
...
class struct_dysymtab_command(MachoFormatter):
    ...
    def __init__(self,data="",offset=0):
        self.func_formatter(cmd=token_cmd_fmt)
        self.address_formatter('tocoff','modtaboff','extrefsymoff',
...
class struct_uuid_command(MachoFormatter):
    ...
    def uuid_fmt(self,k,x,cls=None):
        return highlight([(Token.String,
                          '.'.join(("%02d"%v for v in x)))])
...
class record(object):
    def __init__(self,indx,off,ordinal,typ,flags,addend,s=None):
        ...
        self.symbol = s
    def as_list(self):
        s = (self.seg_index,self.seg_offset,
             self.lib_ordinal,
             self.type,
             self.flags,
             self.addend,
